...
package fl.controls {
     ...
     *
     * @includeExample examples/ComboBoxExample.as -noswf
     *
     * @see List
     * @see TextInput
     *
	...
	public class ComboBox extends UIComponent implements IFocusManagerComponent {
		...
		public function open():void {
            ...
            if (_tween && _tween.isPlaying) {
                _tween.destroy();
                _tween = new Tween(list,"alpha",null,list.alpha,1,0.2,true);
            } else {
                list.alpha = 0;
                _tween = new Tween(list,"alpha",null,0,1,0.2,true);
            }

			list.scrollToSelected();
            if (!focusManager.form.contains(list)) {
		...
		public function close():void {
            ...
            if (_tween && _tween.isPlaying) {
                _tween.destroy();
                _tween = new Tween(list,"alpha",null,list.alpha,0,0.2,true);
                _tween.setFinishCallBack(onTweenC);
            } else {
                _tween = new Tween(list,"alpha",null,1,0,0.2,true);
                _tween.setFinishCallBack(onTweenC);
            }
		}
		...
		override protected function configUI():void {
            ...
            background.addEventListener(MouseEvent.MOUSE_WHEEL, onToggleMouseWheel, false, 0, true);
			addChild(background);
        ...
        protected function onToggleMouseWheel(event:MouseEvent):void {
            event.stopPropagation();
			dispatchEvent(event);
			if (length == 0) return;
            if(event.delta > 0){
